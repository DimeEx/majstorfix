...
describe("JobBadges", () => {
  it("renders house badge", () => {
    render(<JobBadges propertyType="house" floor={null} hasElevator={false} urgency="flexible" tradeType="plumbing" />);
  ...
  it("renders apartment badge with floor info", () => {
    render(<JobBadges propertyType="apartment" floor={3} hasElevator={true} urgency="emergency" tradeType="electrical" />);
  ...
  it("shows no elevator warning", () => {
    render(<JobBadges propertyType="apartment" floor={5} hasElevator={false} urgency="few_days" tradeType="painting" />);
  ...
  it("renders urgency badges with correct text", () => {
    const { rerender } = render(<JobBadges propertyType="house" floor={null} hasElevator={false} urgency="emergency" tradeType="other" />);
    expect(screen.getByText("Итно")).toBeInTheDocument();

    rerender(<JobBadges propertyType="house" floor={null} hasElevator={false} urgency="few_days" tradeType="other" />);
    expect(screen.getByText("2-3 дена")).toBeInTheDocument();

    rerender(<JobBadges propertyType="house" floor={null} hasElevator={false} urgency="flexible" tradeType="other" />);
