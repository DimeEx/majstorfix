...
describe("Database Migration 00004 - Ratings", () => {
  const migrationPath = join(__dirname, "..", "..", "supabase", "migrations", "00004_ratings.sql");
  let sql: string;
  ...
  it("creates ratings table with required columns", () => {
    ...
    expect(sql).toContain("bid_id UUID UNIQUE REFERENCES bids(id) ON DELETE CASCADE");
  ...
  it("creates indexes", () => {
    expect(sql).toContain("CREATE INDEX IF NOT EXISTS idx_ratings_handyman_phone");
  ...
  it("creates RLS policies", () => {
    ...
    expect(sql).toContain(`CREATE POLICY "Job owners can rate bids on their jobs"`);
    expect(sql).toContain(`CREATE POLICY "Reviewers can update their own ratings"`);
    expect(sql).toContain(`CREATE POLICY "Reviewers can delete their own ratings"`);
  });
});
